...
jQuery(function($){
      ...
      jQuery(document).on('click', 'input.select-img', function( event ){
        ...
        var Blog Diary ProImage = wp.media.controller.Library.extend({
            defaults :  _.defaults({
        ...
        blog_diary_file_frame = wp.media.frames.blog_diary_file_frame = wp.media({
          button: {
            text: jQuery( this ).data( 'uploader_button_text' ),
          },
          state : 'tpiw-insert-image',
              states : [
                  new Blog Diary ProImage()
              ],
          multiple: false  // Set to true to allow multiple files to be selected
        });

        // When an image is selected, run a callback.
        blog_diary_file_frame.on( 'select', function() {

          var state = blog_diary_file_frame.state('tpiw-insert-image');

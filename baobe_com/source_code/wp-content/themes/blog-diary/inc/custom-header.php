 ...
 *
 * @uses blog_diary_header_style()
 */
function blog_diary_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'blog_diary_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/custom-header-image.jpg',
		'default-text-color'     => 'ff696a',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'blog_diary_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'blog-diary' ),
		),
	) );
}
add_action( 'after_setup_theme', 'blog_diary_custom_header_setup' );

if ( ! function_exists( 'blog_diary_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see blog_diary_custom_header_setup().
	 */
	function blog_diary_header_style() {
		...
		wp_add_inline_style( 'blog-diary-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'blog_diary_header_style', 10 );
